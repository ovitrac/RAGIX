...
class UnixRAGAgent:
    ...
    def get_history_token_estimate(self) -> int:
        """
        Estimate the number of tokens in the conversation history.
        Uses a simple heuristic: ~4 characters per token (rough average for English).
        """
        total_chars = sum(len(msg.get("content", "")) for msg in self.history)
        return total_chars // 4

    def compact_history(self, keep_recent: int = 4) -> Dict:
        """
        Compact conversation history by summarizing older messages.

        This reduces context usage while preserving key information.
        The summary is generated by the LLM and replaces older messages.

        Args:
            keep_recent: Number of recent messages to keep uncompacted (default: 4)

        Returns:
            Dict with compaction stats: messages_before, messages_after, tokens_saved_estimate
        """
        ...
        # Split history into parts to compact and parts to keep
        messages_to_compact = self.history[:-keep_recent]
        messages_to_keep = self.history[-keep_recent:]

        # Estimate tokens before compaction
        tokens_before = self.get_history_token_estimate()

        # Create a summary prompt
        summary_prompt = """You are a conversation summarizer. Summarize the following conversation history into a concise summary that captures:
1. Key user goals and requests
2. Important decisions made
3. Commands executed and their outcomes
4. Files modified or examined
...
"""
        for msg in messages_to_compact:
            ...
            content = msg.get("content", "")[:1000]  # Truncate very long messages
            summary_prompt += f"\n[{role.upper()}]: {content}\n"
        ...
        try:
            if hasattr(self.llm, 'generate_with_stats'):
                summary, stats = self.llm.generate_with_stats(
            ...
            return {
                "compacted": False,
                "reason": f"Failed to generate summary: {str(e)}",
                "messages_before": len(self.history),
                "messages_after": len(self.history),
                "tokens_saved_estimate": 0
            }

        # Create compacted history with summary as first message
        summary_message = {
            "role": "system",
        ...
        self.history = [summary_message] + messages_to_keep

        # Estimate tokens saved
        tokens_after = self.get_history_token_estimate()
        tokens_saved = max(0, tokens_before - tokens_after)

        # Log the compaction
        if hasattr(self.shell, 'log_event'):
            self.shell.log_event("MEMORY_COMPACTED", f"Compacted {len(messages_to_compact)} messages, saved ~{tokens_saved} tokens")

        return {
            "compacted": True,
            "messages_before": len(messages_to_compact) + len(messages_to_keep),
            "messages_after": len(self.history),
    ...
    def should_compact(self, context_limit: int, threshold_percent: float = 80.0) -> bool:
        """
        Check if history should be compacted based on context usage.

        Args:
            context_limit: Maximum context tokens for the model
            threshold_percent: Trigger compaction at this percentage (default: 80%)

        Returns:
            True if compaction is recommended
        """
        estimated_tokens = self.get_history_token_estimate()
        # Add system prompt estimate
        system_tokens = len(self.system_prompt) // 4
        total_estimate = estimated_tokens + system_tokens

        usage_percent = (total_estimate / context_limit) * 100
        return usage_percent >= threshold_percent

    def get_memory_stats(self) -> Dict:
        """Get current memory/history statistics."""
        return {
            "message_count": len(self.history),
            "estimated_tokens": self.get_history_token_estimate(),
            "system_prompt_tokens": len(self.system_prompt) // 4,
            "total_estimated": self.get_history_token_estimate() + len(self.system_prompt) // 4,
            "oldest_message_role": self.history[0]["role"] if self.history else None,
            "is_compacted": self.history[0].get("content", "").startswith("[CONVERSATION SUMMARY") if self.history else False
        }
    ...
    def step(self, user_text: str) -> Tuple[Optional[CommandResult], Optional[str]]:
        ...
        # Query the LLM for the next action (with token tracking)
        if hasattr(self.llm, 'generate_with_stats'):
            raw, stats = self.llm.generate_with_stats(self.system_prompt, self.history)
            self._update_token_stats(stats)
        else:
