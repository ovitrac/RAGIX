...
Saqqara (document substrate):
32. koas_saqqara_run(source, workspace, formats, promote_outline)
33. koas_saqqara_status(workspace)
...
# ---------------------------------------------------------------------------
# 12b. Saqqara — document substrate
# ---------------------------------------------------------------------------

def _register_saqqara_tools() -> bool:
    """Register the saqqara family's own MCP surface.

    The tools are defined in ragix_kernels/saqqara/mcp/tools.py, beside the kernel
    they expose, like every other family here. Defining them in this file made the
    server the owner of a surface it does not implement.
    """
    try:
        from ragix_kernels.saqqara.mcp.tools import register_saqqara_tools

        register_saqqara_tools(mcp)
        return True
    except Exception as e:  # pragma: no cover - registration is environmental
        import logging
        logging.getLogger(__name__).warning(
            f"Saqqara tools registration failed (continuing without saqqara): {e}"
        )
        return False
