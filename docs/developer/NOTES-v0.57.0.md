> **RAGIX 0.57.0** introduces the **Graph Propagation Algorithm** — a multi-phase directional propagation system for accurate partition classification in large Java codebases.

------

# RAGIX v0.57.0 — Graph Propagation Algorithm
...
|----------|---------|-------------|
| **Algorithm** | Graph Propagation | Multi-phase directional propagation with weighted voting |
| **Algorithm** | Directional Weighting | Forward deps (imports) weighted 0.7, reverse deps (callers) 0.3 |
| **Algorithm** | Package Cohesion | Same-package neighbors receive +0.2 bonus weight |
| **Algorithm** | Multi-Phase Thresholds | Core (0.8) → Near (0.6) → Far (0.4) propagation |
| **Algorithm** | Confidence Decay | -0.05 per iteration prevents over-propagation |
| **Algorithm** | Convergence Detection | Stop when < 1% nodes change per iteration |
| **UI** | Algorithm Parameters | Full exposure of propagation settings in Partitioner tab |
| **UI** | Option Groups | Organized sections for Directional, Cohesion, Thresholds, Convergence |
...
- **v0.56** — Improved dead code detection (isolated classes only)
- **v0.57** — Graph Propagation Algorithm, multi-phase directional classification
